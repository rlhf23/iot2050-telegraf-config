        ...
        if config_manager.xml_files.is_empty() {
            ...
            for (i, file) in config_manager.xml_files.iter().enumerate() {
                ui.group(|ui| {
                    ...
                    let file_config = config_manager.file_configs.entry(file.clone()).or_default();
                    ...
                    egui::Grid::new(&format!("xml_file_grid_{}", i))
                        ...
                        .show(ui, |ui| {
                            ...
                            let has_error = config_manager.form_state.ip_errors.get(file).unwrap_or(&false);
                            ...
                            if response.changed() {
                                config_manager.validate_ip_for_file(file, &file_config.ip);
                            }
                            ui.end_row();
