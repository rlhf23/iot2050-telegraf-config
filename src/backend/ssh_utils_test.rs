...
mod tests {
    ...
    mod file_operation_tests {
        ...
        fn test_send_file_over_ssh_with_empty_file() {
            let dir = tempdir().unwrap();
            let file_path =
                create_test_file(&dir.path().to_path_buf(), "empty.conf", "");
    ...
    fn test_get_telegraf_logs_with_different_line_counts() {
        ...
        for lines in [1, 10, 100, 1000] {
            ...
            assert!(result.is_err(), "Should fail with invalid host regardless of line count");
        }
    }
    ...
    mod container_status_tests {
        ...
        fn test_check_influxdb_status_with_different_timeouts() {
            ...
            for timeout in [1, 5, 10, 30] {
                ...
                assert!(result.is_err() || matches!(result, Ok((false, _))), 
    ...
    mod connection_timeout_tests {
        ...
        fn test_connection_timeout_with_invalid_hostname() {
            ...
            // And it should fail relatively quickly (within a reasonable timeout)
            assert!(elapsed < Duration::from_secs(10), 
                   "DNS resolution took too long to timeout: {:?}", elapsed);
        }
    }
    ...
    mod boundary_tests {
        ...
        fn test_edge_cases() {
            ...
            let result1 = ssh_utils::validate_host_format_exposed_for_testing(&format!("{}:22", long_hostname));
            assert!(result1.is_ok(), "Long but valid hostname should be accepted");
            ...
            let result3 = ssh_utils::validate_host_format_exposed_for_testing("my-host.example-domain.com:22");
            assert!(result3.is_ok(), "Hostname with hyphens and dots should be valid");
        }
    }
}
